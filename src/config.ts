...
const VOICE_STREAM_TIMEOUT = parseInt(process.env.VOICE_STREAM_TIMEOUT || "2000", 10); // 音声ストリームタイムアウト時間（Discord VADで音声が止まった場合の強制送信、デフォルト: 2000ms）
...
console.log(
  `CHAT_COMPLETION_ENDPOINT_URL: ${CHAT_COMPLETION_ENDPOINT_URL || "未設定"}`
);
console.log(
...
console.log(
  `TTS_MODEL: ${TTS_MODEL || "未設定"}`
);
console.log(
  `TTS_VOICE: ${TTS_VOICE || "未設定"}`
);
console.log(
  `TTS_SPEED: ${TTS_SPEED}`
);
